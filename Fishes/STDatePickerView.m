...
- (void)setUpFrame
{
    ...
    // 默认self Y值
    self.toolViewY = ScreenH - ViewH;
    // 默认设置self的Y值在屏幕下方
    self.frame = CGRectMake(0, ScreenH, ScreenW, ViewH);
}
...
- (void)show
{
    UIView *screenView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, ScreenW, ScreenH)];
    screenView.tag     = 1001;
    ...
    [UIView animateWithDuration:0.35 animations:^
     {
         ...
         self.frame = CGRectMake(0, self.toolViewY, ScreenW, self.datePicker.frame.size.height + ToolbarH);
...
- (void)remove
{
    [UIView animateWithDuration:0.35 animations:^
     {
         self.frame = CGRectMake(0, ScreenH, ScreenW, self.datePicker.frame.size.height + ToolbarH);
...
- (UIDatePicker *)datePicker
{
    if (!_datePicker)
    {
        ...
        // UIDatePicker默认高度216
        _datePicker.frame = CGRectMake(0, ToolbarH , ScreenW, _datePicker.frame.size.height);
    }
    return _datePicker;
}
...
- (UIToolbar *)toolBar
{
    if (!_toolBar)
    {
        _toolBar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, 0, ScreenW, ToolbarH)];
