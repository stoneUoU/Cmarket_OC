...
@interface STDropMenu()<UITableViewDelegate,UITableViewDataSource>
...
- (instancetype)initWithFrame:(CGRect)frame ArrowOffset:(CGFloat)arrowOffset TitleArr:(NSArray *)titleArr ImageArr:(NSArray *)imageArr Type:(STDropMenuType)type LayoutType:(STDropMenuLayoutType)layoutType RowHeight:(CGFloat)rowHeight Delegate:(id<STDropMenuDelegate>)delegate andSelArr:(NSMutableArray *)selArr{
    if (self = [super initWithFrame:frame]) {
        ...
        for (int i = 0; i < titleArr.count; i++) {
            ...
            STDropMenuModel *model = [STDropMenuModel dropMenuWithDictonary:tempDict];
...
+ (instancetype)showDropMenuFrame:(CGRect)frame ArrowOffset:(CGFloat)arrowOffset TitleArr:(NSArray *)titleArr ImageArr:(NSArray *)imageArr Type:(STDropMenuType)type LayoutType:(STDropMenuLayoutType)layoutType RowHeight:(CGFloat)rowHeight Delegate:(id<STDropMenuDelegate>)delegate andSelArr:(NSMutableArray *)selArr{
    return [[self alloc] initWithFrame:frame ArrowOffset:arrowOffset TitleArr:titleArr ImageArr:imageArr Type:type LayoutType:layoutType RowHeight:rowHeight Delegate:delegate andSelArr:selArr];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    STDropMenuCell *cell = [STDropMenuCell dropMenuCellWithTableView:tableView];
    ...
    if ([_selArr[indexPath.row]  isEqual: @1]){
        cell.titleL.textColor = styleColor;
    }else{
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    STDropMenuModel *model = self.titleImageArrM[indexPath.row];
...
#pragma mark - 类型
- (void)setLayoutType:(STDropMenuLayoutType)LayoutType {
    _LayoutType = LayoutType;
    for (STDropMenuModel *model in self.titleImageArrM) {
...
+ (instancetype)dropMenuCellWithTableView:(UITableView *)tableView {
    STDropMenuCell *cell = [tableView dequeueReusableCellWithIdentifier:kDropMenuCellID];
    if (!cell) {
        cell = [[STDropMenuCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kDropMenuCellID];
    }
    return cell;
}
...
- (void)layoutSubviews {
    ...
    if (self.model.layoutType == STDropMenuLayoutTypeTitle) {
...
- (void)setModel:(STDropMenuModel *)model {
    _model = model;
    if (model.layoutType == STDropMenuLayoutTypeTitle) {
        self.titleL.frame = CGRectMake(10, self.screenCenter - 10, self.frame.size.width, 20);
    } else {
        self.imageIV.image = [UIImage imageNamed:model.image];
    }
    if (model.type == STDropMenuTypeQQ) {
        self.titleL.textColor = [UIColor blackColor];
        self.line1.backgroundColor = [UIColor lightGrayColor];
    } else {
...
@interface STDropMenuModel()
@end
@implementation STDropMenuModel
