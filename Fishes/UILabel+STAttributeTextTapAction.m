//
//  UILabel+STAttributeTextTapAction.m
//  Fishes
//
...
@implementation STAttributeModel

@end

@implementation UILabel (STAttributeTextTapAction)
...
- (id<STAttributeTapActionDelegate>)delegate
{
    return objc_getAssociatedObject(self, _cmd);
}
...
- (void)setDelegate:(id<STAttributeTapActionDelegate>)delegate
{
    objc_setAssociatedObject(self, @selector(delegate), delegate, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

#pragma mark - mainFunction
- (void)st_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings tapClicked:(void (^) (NSString *string , NSRange range , NSInteger index))tapClick
{
    [self st_getRangesWithStrings:strings];
...
- (void)st_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings
                                   delegate:(id <STAttributeTapActionDelegate> )delegate
{
    [self st_getRangesWithStrings:strings];
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    [self st_getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {

        if (weakSelf.tapBlock) {
            weakSelf.tapBlock (string , range , index);
        }

        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(st_attributeTapReturnString:range:index:)]) {
            [weakSelf.delegate st_attributeTapReturnString:string range:range index:index];
        }

        if (self.isTapEffect) {

            [self st_saveEffectDicWithRange:range];

            [self st_tapEffectWithStatus:YES];
        }
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (self.isTapAction) {
        if ([self st_getTapFrameWithTouchPoint:point result:nil]) {
            return self;
        }
    }
...
- (BOOL)st_getTapFrameWithTouchPoint:(CGPoint)point result:(void (^) (NSString *string , NSRange range , NSInteger index))resultBlock
{
    ...
    CGAffineTransform transform = [self st_transformForCoreText];
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        CGRect flippedRect = [self st_getLineBounds:line point:linePoint];
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            for (int j = 0; j < link_count; j++) {
                ...
                STAttributeModel *model = self.attributeStrings[j];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    if (self.isTapEffect) {
        ...
        [self performSelectorOnMainThread:@selector(st_tapEffectWithStatus:) withObject:nil waitUntilDone:NO];
...
- (CGAffineTransform)st_transformForCoreText
{
    return CGAffineTransformScale(CGAffineTransformMakeTranslation(0, self.bounds.size.height), 1.f, -1.f);
}

- (CGRect)st_getLineBounds:(CTLineRef)line point:(CGPoint)point
{
    CGFloat ascent = 0.0f;
...
- (void)st_tapEffectWithStatus:(BOOL)status
{
    if (self.isTapEffect) {
...
- (void)st_saveEffectDicWithRange:(NSRange)range
{
...
- (void)st_getRangesWithStrings:(NSArray <NSString *>  *)strings
{
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            totalStr = [totalStr stringByReplacingCharactersInRange:range withString:[weakSelf st_getStringWithRange:range]];

            STAttributeModel *model = [STAttributeModel new];
...
- (NSString *)st_getStringWithRange:(NSRange)range
{
...
@interface UILabel (STAttributeTextTapAction)
